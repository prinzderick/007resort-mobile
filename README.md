...
| **Attendant - payment collection** | Waiters | **Waiter payment collection** (see below): order card shows the bill state, **Take payment** at the table (cash / card machine / transfer / pay link, split across tenders), a clear status per collection, **My cash** and hand-over to the cashier. |
